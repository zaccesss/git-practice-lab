# 04: Rebase and History

I use this module to practise cleaning up my commit history before I share it, using the tools that rewrite history rather than the ones that add to it.
...
8. Separately, practise fixing a typo in the file you just committed without creating a new commit: edit the file, `git add` it, then `git commit --amend --no-edit`.

## Questions to answer for yourself

- Why is rebasing safe on a branch only you are working on, but risky on a branch others have already pulled?
- What is the difference between `git rebase` and `git merge` when bringing `main` into a feature branch?
- What does `git reflog` show? How could it save you if an interactive rebase went wrong?

## Why this matters to me

A messy, exploratory commit history is fine while I am working. Before I open a pull request, a clean history makes the diff easier to review and the log easier for me to read months later.
